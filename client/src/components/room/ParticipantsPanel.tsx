...
import { X, Users, Mic, MicOff, Camera, CameraOff, Search, Pin, Shield } from "lucide-react";
    ...
    <div className="w-full md:w-[360px] h-full shrink-0 border-l border-white/5 bg-slate-900 flex flex-col relative z-40 select-none">
      ...
      <div className="h-16 shrink-0 flex items-center justify-between px-4 border-b border-white/5">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm">People ({allList.length})</span>
        </div>
          ...
          {filtered.map((p) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-white/90 truncate flex items-center gap-1">
                      <span>{isSelf ? `${p.name} (You)` : p.name}</span>
                      {p.isHost && (
                        <Shield className="h-3 w-3 text-primary shrink-0" title="Meeting Host" />
                      )}
                    </p>
                ...
                {/* Status indicators and action pins */}
                <div className="flex items-center gap-1.5 shrink-0">
                  {/* Pin button */}
                  <Button
                    onClick={() => onPinToggle(p.id)}
                    variant="ghost"
                    size="icon"
                  ...
                  >
                    <Pin className={`h-3.5 w-3.5 ${pinnedId === p.id ? "text-primary fill-primary" : ""}`} />
                  </Button>
